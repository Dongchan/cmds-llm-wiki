...
- If an **academic paper** (DOI/arXiv/journal URL, PDF with Abstract+References, or a file from `00. Inbox/02. Papers/`): **use Paper Ingest Mode** — read `90. Settings/Skills/paper-ingest.md` for the full 12-step pipeline (do NOT inline it here — progressive disclosure keeps this command lean).
...
- **Read the resource file** `90. Settings/Skills/paper-ingest.md` for the full P-0 → P-7 pipeline (12-step schemes, citekey naming, RQ linking, `p7_verify.py` gate). Do NOT inline it here — progressive disclosure keeps this command lean.
